...
def reset_show(show):
    ...
    if not show in config.sections():
...
def main():
    ...
    for cfg_name in shows:
        ...
        for season in range(int(start_season), num_seasons+1):
            ...
            ordkey = lambda e: _parse_summary(e['summary'])['episode'] * 100 + e['ord']
        ...
        for entry in entries:
            ...
            try:
                ...
                if '"duplicate torrent"' in str(e):
                    ...
                    trans_info = trans_client.inf(hash.hexdigest())
                    trans_client.start(trans_info.keys()[0])
                else:
                    raise
...
if __name__ == "__main__":
    #parse options
    option_list = [
        make_option('--reset-show', action='store', type='string', help="Delete a show's download history", metavar="SHOW_NAME"),
    ]
